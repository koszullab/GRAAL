...
class ResultEventTmpGraal(wx.PyEvent):
    """Simple event to carry arbitrary result data."""
    def __init__(self, gl_window):
        """Init Result Event."""
        wx.PyEvent.__init__(self)
...
class GraalThread(Thread):
    ...
    def __init__(self, notify_window,):
        ...
        self.id_level = notify_window.id_level
        self.n_iterations = notify_window.n_iterations
        self.n_burnin = notify_window.n_burnin
        ...
        self.threshold_coverage = notify_window.threshold_coverage
    ...
    def run(self):
        ...
        self.gl_window = main_gl.window(self.pyramid, self.name, self.id_level, self.n_iterations, self.is_simu,
                                        self.scrambled,
                                        self.perform_optim, self.sample_parameters, self.output_folder, self.fasta_file,
                                        self.blacklisted_contig, self.n_neighbours, self.threshold_coverage,
                                        self.allow_repeats, self.id_selected_gpu,
                                        self)

        self.start_EM()
        # self.gl_window.start_EM()
        # self.gl_window.simulation.release()
        wx.PostEvent(self._notify_window, ResultEventGraal(self.gl_window))


    def start_EM(self,):
        print "start expectation maximization ... "
        delta = np.ones((self.gl_window.n_iterations_em,),dtype=np.int32) * self.gl_window.n_neighbours
        o, d, d_high = self.gl_window.simulation.sampler.display_current_matrix(self.gl_window.simulation.input_matrix)
        self.gl_window.simulation.sampler.init_likelihood()
        self.gl_window.simulation.sampler.modify_gl_cuda_buffer(0, self.gl_window.dt)

        if self.gl_window.scrambled:
            self.gl_window.simulation.sampler.explode_genome(self.gl_window.dt)
        o, d, d_high = self.gl_window.simulation.sampler.display_current_matrix(self.gl_window.simulation.scrambled_input_matrix)
        list_frags = np.arange(0, self.gl_window.simulation.sampler.n_new_frags, dtype=np.int32)
        self.gl_window.iter = 0
        n_iter = np.float32(self.gl_window.n_iterations_em)
        self.gl_window.bins_rippe = self.gl_window.simulation.sampler.bins
        for j in xrange(0, self.gl_window.n_iterations_em):
            print "cycle = ", j
            self.gl_window.str_curr_cycle = "current cycle = " + str(j)
            np.random.shuffle(list_frags)

            for itmp in range(0, self.gl_window.simulation.sampler.n_new_frags):
                i = list_frags[itmp]
                # print "id_frag =", i
                self.gl_window.curr_frag = i
                self.gl_window.remote_update()

                o, n_contigs, min_len, mean_len, max_len, op_sampled, id_f_sampled, dist, temp = \
                    self.gl_window.simulation.sampler.step_max_likelihood(i,
                                                                         delta[j],
                                                                         512, self.gl_window.dt,
                                                                         np.float32(j), n_iter)

                self.gl_window.str_likelihood = "likelihood = " + str(o)
                self.gl_window.str_n_contigs = "n contigs = " + str(n_contigs)
                self.gl_window.str_curr_id_frag = "current frag = "+ str(i)
                self.gl_window.str_curr_dist = "current dist = "+ str(dist)
                self.gl_window.str_curr_temp = "current temperature = "+str(temp)
                self.gl_window.str_curr_d = "current d = " + str(d)

                self.gl_window.collect_full_likelihood.append(self.gl_window.simulation.sampler. likelihood_t)
                self.gl_window.collect_likelihood.append(o)
                self.gl_window.collect_n_contigs.append(n_contigs)
                self.gl_window.collect_mean_len.append(mean_len)
                self.gl_window.collect_op_sampled.append(op_sampled)
                self.gl_window.collect_id_fB_sampled.append(id_f_sampled)
                self.gl_window.collect_id_fA_sampled.append(i)
                self.gl_window.collect_dist_from_init_genome.append(dist)
                self.gl_window.iter += 1
                # sampling nuisance parameters
                if self.gl_window.sample_param and j > self.n_burnin:
                    fact, d, d_max, d_nuc, slope, likeli, success, y_eval = \
                        self.gl_window.simulation.sampler.step_nuisance_parameters(self.gl_window.dt, np.float32(j),
                                                                                                    n_iter)
                    self.gl_window.y_eval = y_eval
                else:
                    success = 1
                    curr_param = np.copy(self.gl_window.simulation.sampler.param_simu)
                    kuhn, lm, c1, slope, d, d_max, fact, d_nuc = curr_param[0]
                    likeli = o
                mean_len_kb = mean_len / 1000.
                self.update_gui(o, n_contigs, mean_len_kb, dist, slope, d_max)
                if itmp%50 == 0:
                    self.gl_window.simulation.sampler.update_vect_model()
                    self.update_gui_parameters(self.gl_window.simulation.sampler.bins_upd,
                                                           self.gl_window.simulation.sampler.mean_contacts_upd,
                                                           self.gl_window.simulation.sampler.y_estim)

                self.gl_window.collect_fact.append(fact)
                self.gl_window.collect_d.append(d)
                self.gl_window.collect_d_max.append(d_max)
                self.gl_window.collect_d_nuc.append(d_nuc)
                self.gl_window.collect_slope.append(slope)
                self.gl_window.collect_likelihood_nuisance.append(likeli)
                self.gl_window.collect_success.append(success)

            o, d, d_high = self.gl_window.simulation.sampler.display_current_matrix(self.gl_window.simulation.output_matrix_em)
            self.gl_window.simulation.export_new_fasta()
        self.gl_window.save_behaviour_to_txt()
    ...
    def update_gui_parameters(self, kb_bins, obs_contacts, model_contacts):
        wx.CallAfter(self._notify_window.redraw_parameters, kb_bins, obs_contacts, model_contacts)
...
class GraalFrame(wx.Frame):
    ...
    def __init__(self,*args, **kwargs):
        ...
        self.list_n_neihbours = [str(i) for i in range(3, 20)]
        self.list_n_std_cov = [str(i) for i in range(-1, 10)]
        self.list_n_burnin = [str(i) for i in range(1, 5)]
    ...
    def layout_parameters(self):
        ...
        self.gs_param = wx.FlexGridSizer(10, 2, 20, 20)
        ################################################################################################################
        ...
        self.cmb_n_neighbours = wx.ComboBox(self.panel, -1, value="3", choices=self.list_n_neihbours,
                                              style=wx.CB_READONLY, )
        ################################################################################################################
        self.txt_n_std_cov = wx.StaticText(self.panel, -1, _("N std coverage:"))
        self.cmb_n_std_cov = wx.ComboBox(self.panel, -1, value="0", choices=self.list_n_std_cov,
                                              style=wx.CB_READONLY, )

        ################################################################################################################
        self.txt_n_burnin = wx.StaticText(self.panel, -1, _("Burn-In cycles:"))
        self.cmb_n_burnin = wx.ComboBox(self.panel, -1, value="2", choices=self.list_n_burnin,
                                              style=wx.CB_READONLY, )
        ################################################################################################################
        ...
        list_param = [self.txt_sub_sample_lev,self.cmb_sub_sample_lev,
                      ...
                      self.txt_n_std_cov, self.cmb_n_std_cov,
                      self.txt_n_burnin, self.cmb_n_burnin,
    ...
    def on_start_button(self, event):
        ...
        self.id_level = int(self.cmb_sub_sample_lev.GetValue())
        self.n_iterations = int(self.text_ctrl_n_cycles.GetValue())
        self.n_neighbours = int(self.cmb_n_neighbours.GetValue())
        self.fact_thresh_cov = np.float32(self.cmb_n_std_cov.GetValue())
        self.threshold_coverage = self.frags_coverage.mean() + self.frags_coverage.std() * self.fact_thresh_cov
        self.n_burnin = int(self.cmb_n_burnin.GetValue())
    ...
    def init_plot_coverage(self):
        self.id_level = self.n_level - 1
        self.level = self.pyramid.get_level(self.id_level)
        self.sub_level = self.pyramid.get_level(self.id_level - 1)
        self.mat = self.level.sparse_mat_csr
        self.sub_mat = self.sub_level.sparse_mat_csr
        ...
        self.frags_coverage = coverage
        self.threshold_coverage = coverage.mean() + coverage.std() * 0
        self.id_max_frags = self.mat.shape[0]

        self.id_frags = np.arange(0, self.id_max_frags)
        self.dpi = 80
        self.fig_coverage = Figure((3.0, 3.0), dpi=self.dpi)

        self.axes_coverage = self.fig_coverage.add_subplot(111)
        self.axes_coverage.set_axis_bgcolor('black')
        self.axes_coverage.set_title('Coverage per bin', size=8)

        pylab.setp(self.axes_coverage.get_xticklabels(), fontsize=8)
        pylab.setp(self.axes_coverage.get_yticklabels(), fontsize=8)

        self.axes_coverage.set_xlabel('Bins Id', fontsize=8)
        self.axes_coverage.set_ylabel('Coverage (contacts)', fontsize=8)

        self.plot_data_coverage = self.axes_coverage.plot(self.id_frags, self.frags_coverage, 'r')[0]
        self.plot_thresh_coverage = self.axes_coverage.axhline(self.threshold_coverage, color='c', linestyle='--')

        pylab.setp(self.axes_coverage.get_yticklabels(), fontsize=8)

    def draw_plot_coverage(self):

        self.axes_coverage.set_xbound(lower=0, upper=np.max(self.id_frags))
        self.axes_coverage.set_ybound(lower=0, upper=np.max(self.frags_coverage))

        self.axes_coverage.grid(True, color='gray')
        pylab.setp(self.axes_coverage.get_xticklabels(),
            visible=True)
        self.plot_data_coverage.set_xdata(self.id_frags)
        self.plot_data_coverage.set_ydata(self.frags_coverage)

        self.plot_thresh_coverage.set_ydata(self.threshold_coverage)
    ...
    def init_plot_model(self):
        ...
        self.axes_model.set_xlabel('Genomic distance (kb)', fontsize=8)
        self.axes_model.set_ylabel('N contacts', fontsize=8)

        self.estimate_parameters()
        # self.obs_contacts = np.arange(1000, 10, -1)
        # self.model_contacts = np.arange(1000, 10, -1) + 5000
        # self.kb_bins = np.arange(10, 1000)

        self.plot_data_obs = self.axes_model.loglog(self.kb_bins, self.obs_contacts, 'g')[0]
    ...
    def draw_plot_model(self):
        ...
        self.axes_model.set_xbound(lower=0, upper=self.kb_bins.max())
        self.axes_model.set_ybound(lower=np.min([self.model_contacts.min(), self.obs_contacts.min()]),
                                   upper=np.max([self.model_contacts.max(), self.obs_contacts.max()]))
        self.axes_model.grid(True, color='gray')
        pylab.setp(self.axes_model.get_xticklabels(),
            visible=True)
        self.plot_data_obs.set_xdata(self.kb_bins)
        self.plot_data_obs.set_ydata(self.obs_contacts)

        self.plot_data_mod.set_xdata(self.kb_bins)
        self.plot_data_mod.set_ydata(self.model_contacts)
    ...
    def on_redraw_timer(self, event):
        ...
        #
        self.draw_plot()
        new_level = int(self.cmb_sub_sample_lev.GetValue())
        self.fact_thresh_cov = int(self.cmb_n_std_cov.GetValue())
        if self.id_level != new_level:
            self.id_level = new_level
            self.level = self.pyramid.get_level(self.id_level)
            self.sub_level = self.pyramid.get_level(self.id_level - 1)
            self.mat = self.level.sparse_mat_csr
            self.sub_mat = self.sub_level.sparse_mat_csr
    ...
    def estimate_parameters(self,):
        ...
        id_start = np.nonzero(self.sub_level.S_o_A_frags['start_bp'] == 0)[0]
        max_dist_kb = self.sub_level.S_o_A_frags['l_cont_bp'][id_start].max() / 1000.
        print "max dist kb = ", max_dist_kb
        # size_bin_kb = self.level.S_o_A_frags['len_bp'].mean() / 1000.0
        size_bin_kb = self.level.S_o_A_frags['len_bp'].mean() / 10000.0
        ...
        for k in self.bins:
            self.dict_collect[k] = []
        self.sub_n_frags = self.sub_level.S_o_A_frags['id_c'].shape[0]
        for i in xrange(0, self.sub_n_frags - 1):
            id_c_i = self.sub_level.S_o_A_frags['id_c'][i]
            start_fi = self.sub_level.S_o_A_frags['start_bp'][i]
            len_fi = self.sub_level.S_o_A_frags['len_bp'][i]
            pos_i = self.sub_level.S_o_A_frags['pos'][i]
            for j in xrange(i + 1, self.sub_n_frags):
                id_c_j = self.sub_level.S_o_A_frags['id_c'][j]
                if id_c_i == id_c_j:
                    ncontacts = self.sub_mat[i, j]
                    start_fj = self.sub_level.S_o_A_frags['start_bp'][j]
                    len_fj = self.sub_level.S_o_A_frags['len_bp'][j]
                    pos_j = self.sub_level.S_o_A_frags['pos'][j]
                    if pos_i < pos_j:
        ...
        self.kb_bins = self.bins_upd
        self.obs_contacts = self.mean_contacts_upd
        self.model_contacts = self.y_estim
